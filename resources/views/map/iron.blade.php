...
@foreach($regiondata as $region)
	...
	@if($region->metal_count >=1)
		<a xlink:href="/region/{{ $region->region_id }}"><title>{{ $region->region_name }}, ({{ $region->metal_count }} iron)</title>
		<circle cx="{{ $region->region_x }}" cy="{{ $region->region_y }}" r="{{ 5 + $region->metal_count }}" stroke="rgb(128,0,0)" stroke-width="{{ 2 + $region->metal_count }}" fill="rgb(58,60,64)" />
		</a>
	@else
