...
@endsection
@section('content')
					<h1>World Map (Working Animal View)</h1>
...
@foreach($regiondata as $region)
	...
	@if($region->ride =="camel")
		<a xlink:href="/region/{{ $region->region_id }}"><title>{{ $region->region_name }}, (camel caravan)</title>
		<circle cx="{{ $region->region_x }}" cy="{{ $region->region_y }}" r="6" stroke="rgb(255,0,255)" stroke-width="2" fill="rgb(128,64,0)" />
		</a>
	@elseif($region->ride =="stud")
		<a xlink:href="/region/{{ $region->region_id }}"><title>{{ $region->region_name }}, (stud farm)</title>
		<circle cx="{{ $region->region_x }}" cy="{{ $region->region_y }}" r="6" stroke="rgb(255,0,0)" stroke-width="2" fill="rgb(128,64,0)" />
		</a>
	@else
